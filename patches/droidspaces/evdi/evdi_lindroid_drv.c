...
/*
 * EVDI creates no display until userspace writes to its sysfs add node.
 * Register it after the early display boot path while keeping it built-in,
 * so the feature remains available in the Image-only AnyKernel package.
 */
late_initcall(evdi_init);
